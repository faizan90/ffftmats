...
def main():
    ...
    for i in range(sims_df.shape[0]):
        ...
        # diffs = diag_vals - step_vals
        #
        # diffs[diffs > 0] = -np.inf
        #
        # x_min_idx = np.argmax(diffs)
    ...
    plt.plot(asymm_ser, c='r', alpha=0.5, label='asymm1')
    ...
    plt.savefig('sims_asymm1.png', bbox_inches='tight', dpi=150)
    plt.close()

    sims_df['asymm1'] = asymm_ser

    sims_df.to_csv(r'sims_asymm1.csv', sep=';')
    return
